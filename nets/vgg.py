...
def VGG16(pretrained, in_channels=3, **kwargs):
    ...
    if pretrained:
        ...
        # 适应地更新模型的 state_dict 以排除 avgpool 和 classifier 的权重
        state_dict = {k: v for k, v in state_dict.items() if not k.startswith("classifier.") and not k.startswith("avgpool.")}
        model.load_state_dict(state_dict, strict=False)

    # 注意：由于您已经在模型定义中删除了这些模块，这里就不需要再删除它们了
    return model
